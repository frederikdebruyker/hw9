...
SELECT 
    last_name 'last name', #header change not required per question but aligning with next exercise
    COUNT(*) AS count
FROM
    actor
GROUP BY last_name
ORDER BY count DESC; # this is not required per question but useful to validate next question
...
# 4c.	The actor `HARPO WILLIAMS` was accidentally entered in the `actor` table as `GROUCHO WILLIAMS`. Write a query to fix the 
# 		record.
UPDATE actor 
SET 
    first_name = 'HARPO'
...
## Note that depending on the interpretation the result could be very different:
## - if we go for a binary comparison, the answer is 0
## - if we go for a non-binary comparison and the default setup, the select is not case sensitive. Result below:
SELECT 
...
# 		by last name:
## A left join was considered for this question in case we have customers who were entered into the system but never paid anything. I assumed
## this was not part of our question.
SELECT
...
#		and `Q` have also soared in popularity. Use subqueries to display the titles of movies starting with the letters `K` and `Q` whose language is 
#		English.
## I included 2 options. I believe option 1 is the way to resolve the issue, but if I go to the letter of the question, I added option 2:
### option 1 with subquery for the language 'English'.
SELECT
    title
FROM
    film
WHERE
    (title LIKE 'K%' OR title LIKE 'Q%')
        ...
        WHERE
            name = 'English');
### option 2 with subqueries on title and language would include a double select on title

# 7b.	Use subqueries to display all actors who appear in the film `Alone Trip`.
SELECT 
    actor.last_name 'last name', actor.first_name 'first name'
FROM
    film_actor
        INNER JOIN
...
# 7c.	You want to run an email marketing campaign in Canada, for which you will need the names and email addresses of all Canadian customers. Use 
...
# 8a.	In your new role as an executive, you would like to have an easy way of viewing the Top five genres by gross revenue. Use the solution from 
#		the problem above to create a view. If you haven't solved 7h, you can substitute another query to create a view.
CREATE VIEW 8a_v AS
    SELECT 
        category.name 'top five genres',
        SUM(payment.amount) 'gross revenue'
